...
class DataArguments:
    ...
    dataset_dir: Optional[str] = field(
        default=None,
        metadata={"help": "Directory of the dataset."}
    )

@dataclass
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    cache_dir: Optional[str] = field(
        default="/data/user_name_data/user_name/HF_cache",
    ...
    ddp_find_unused_parameters: bool = field(
        default=False,
        metadata={"help": "Whether to use find_unused_parameters in DDP."}
    )
    ...
    # output_dir: str = field(default='./data/Verilog_code_generation/output', metadata={"help": 'The output dir for logs and checkpoints'})
    output_dir: str = field(default='./data/Verilog_code_generation/llm1_high_level_summary_to_block_summary_dataset_skip_single_blocks_usage_summary_combined', metadata={"help": 'The output dir for logs and checkpoints'})
    optim: str = field(default='paged_adamw_32bit', metadata={"help": 'The optimizer to be used'})
    per_device_train_batch_size: int = field(default=1, metadata={"help": 'The training batch size per GPU. Increase for better speed.'})
    gradient_accumulation_steps: int = field(default=16//4, metadata={"help": 'How many gradients to accumulate before to perform an optimizer step'})
...
def get_accelerate_model(args, checkpoint_dir):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        tokenizer_type="code_llama" if "codellama" in args.model_name_or_path else ("llama" if 'llama' in args.model_name_or_path else None),
    ...
    elif 'llama' in args.model_name_or_path or isinstance(tokenizer, LlamaTokenizer):
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name):
        ...
        elif dataset_name == 'verilog_baseline':
            print('using baseline data')
            assert args.dataset_dir is not None
            return load_from_disk(args.dataset_dir)
    ...
    def format_dataset(dataset, dataset_format):
        ...
        elif dataset_format == 'verilog_baseline':
            print('using baseline format')
            for old, new in [['code', 'output'], ['description', 'input']]:
